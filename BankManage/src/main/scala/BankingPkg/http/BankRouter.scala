...
import BankingPkg.Actors.PersistentBankAccount.Command.{CreateBankAccount, UpdateBalance}
import BankingPkg.Actors.PersistentBankAccount.{Command, Response}
import BankingPkg.http.Validation.{ValidationResult, Validator, validateEntity, validateMinimum, validateMinimumAbs, validateRequired}
...
import akka.http.scaladsl.server.Directives.{complete, onSuccess, pathEndOrSingleSlash}
...
import BankingPkg.Actors.PersistentBankAccount.Command._
import BankingPkg.Actors.PersistentBankAccount.Response
import BankingPkg.Actors.PersistentBankAccount.Response._
...
object BankAccountCreationRequest {
  implicit val validator: Validator[BankAccountCreationRequest] = new Validator[BankAccountCreationRequest] {
    ...
    //def validateMinimumAbs(balance: Double, d: Double, str: String): Validated[EE_, AA_] = ???
